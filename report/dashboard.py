...
class BarChart(MatplotlibViz):
    ...
    def visualization(self, entity_id, model):
        ...
        # learning model
        model_data_df = model.model_data(entity_id)
        
        # Using the predictor class attribute
        # pass the data to the `predict_proba` method
        probabilities =  self.predictor.predict_proba(model_data_df)
        
        # Index the second column of predict_proba output
        # The shape should be (<number of records>, 1)
        #probabilities_df = pd.DataFrame( probabilities )
        #probabilities_df.set_index(1, inplace=True)
        ...
        if model.name == 'team':
            #pred = probabilities_df.mean().iloc[0]
            pred = probabilities.mean()
            
        # Otherwise set `pred` to the first value
        # of the predict_proba output
        else:
            #pred = probabilities_df.iloc[0,0]
...
# called `Report`
# I think the plural was an error
...
print('Header')
header_ = Header()
print(  header_.build_component(1, Employee())   )


print('NotesTable')
notes_table = NotesTable()
print( notes_table.component_data(1, Employee()) )


print('get')
print(   report.children[0].build_component(14, Employee()) )

print(  report(1, Employee()))
